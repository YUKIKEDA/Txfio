...
| Phase 0 リポジトリ基盤 | 完了 |
| Phase 1 MVP | 完了 |
| Phase 2 並行性・ロック | 進行中（次は Recover とロック Issue #29） |
| Phase 3 拡張と公開 | 未着手 |

Phase 1 の「使えるライブラリ」は main にある。パスロック（Issue #27）も main にある。次は Recover がロックファイルを触らないこと（Issue #29）。そのあと別プロセスの同時アクセスが残る。
...
- Issue #29: Recover は `.lock` を開かず消さない（クラッシュ後もファイルは残り、別トランザクションが取り直せる）
- [ ] 複数プロセスからの同時アクセスのテスト
