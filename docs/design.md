...
各操作は、対象パスの現在の状態を `Before`（未適用）・`After`（適用済み）と照合する。マーカーがあり `After` と一致する操作は `.txnew` を消してスキップし、`Before` と一致する操作は再実行する。どちらとも一致しない場合は外部競合として扱う（「コミット時の外部干渉への対処」に従う）。文書が読めるとき、マーカーが無ければ未コミットとして安全にロールバック（`.txnew` とジャーナルエントリの破棄、および `CreateDirectory` の再帰削除）できる。読めないときは「読めないジャーナル」に従う
