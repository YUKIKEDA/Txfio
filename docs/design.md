...
- ワークフォルダ自身とメタデータフォルダ（`.txfio`）は削除対象外（エラー）
...
2. 各ファイルへの確定処理を実行する。事前に、同一パスへの複数操作をジャーナル記録時点で正規化しておく（例: 同一パスへの`Add`の後に`Delete`が来た場合は両方を打ち消し合う。`Move(A→B)`の後に`Move(B→C)`が来た場合は`Move(A→C)`に畳み込む）。正規化後の操作について、依存関係のあるパターン（例: `Move(A→B)`と同時に新規`Add(A)`のような入れ替わり）が検出された場合はコミット前バリデーションでエラーとし、非破壊的操作（Add新規作成、Move）を先に・破壊的操作（Update上書き、Delete実削除）を後にという単純な順序のみサポートする。Delete のうちディレクトリは、ファイル Delete と Move 出しのあと、パスが深い順に非再帰で消す（親が空になってから）。依存関係を考慮したトポロジカルソートは将来の拡張候補とする。各操作について、適用前の期待状態（Before: 変更前のサイズ・最終更新日時）と適用後の期待状態（After: 変更後のサイズ・最終更新日時）をジャーナルに記録する。`Applying`/`Applied`ステータスの書き込み自体はfsyncせず（`Committing`マーカー書き込み時の1回のみfsyncする）、Recover時は対象パスの実際の状態をBefore/Afterと突き合わせて適用済みかどうかを判定する
