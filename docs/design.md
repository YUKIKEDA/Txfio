...
- エラーハンドリングは例外ベース。基底は `TxfioException`。ディスク上の前提が崩れたときは `ExternalConflictException`（失敗したパスを 1 つ持つ。対象が無い、既にある、移動先がディレクトリ、親やワークフォルダが無い、ディレクトリ直下に予定外の子がある、Export のコピー先が塞がっている）。未対応（ディレクトリの Move / Attach / Import / Export、ボリュームをまたぐ Move）は `UnsupportedOperationException`。使い方の誤りは `InvalidOperationException` と `ArgumentException` のまま。他のトランザクションがパスを押さえているときは `LockContentionException`（失敗したパスを 1 つ持つ。内部例外は持たない）。コミットの成否は例外にせず `CommitResult` で返す
...
- ディレクトリのMoveは、配下ファイルの個別ロックとの競合を確実に検知する手段（子孫探索なしでは発見できない）がないため、MVPでは非サポートとする（ファイルの直接Move/Renameのみサポート）。将来的にサポートする場合は、ディレクトリMove中はそのトランザクションがワークフォルダ全体の排他ロックを取得する（Move中は他の全トランザクションを止める）という割り切りで拡張する
...
**ロック粒度**: 操作が名指ししたパスだけをロックする。ファイルでもディレクトリでも、そのパスの `.txfio/locks/{16進}.lock` を作る。ハッシュする文字列はワークフォルダからの相対パスで、区切りは `\`、`ToUpperInvariant` で畳んだ UTF-8 の SHA-256 である。ディレクトリの Delete はそのディレクトリだけで、子はロックしない。異なるパスを触るトランザクション同士は並行実行できる。ワークフォルダ全体の排他ロックは、全ての並行実行パターンをサポートする要件下では実質シリアライズになってしまうため採用しない（ただしディレクトリMoveは例外。後述）
...
**Move操作時のロック**: 旧パス・新パスの両方に対してロックを取得する（他のトランザクションが移動先パスに書き込もうとする競合を防ぐため）。取得順序は、大文字化した絶対パスの辞書順とする（`Move(A→B)`と`Move(B→A)`が同時に走ってもデッドロックしない）。2本目が取れなくても、1本目は持ち続ける。同じパスへの Move はロックしない
