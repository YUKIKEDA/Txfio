...
**テスト戦略**: 通常のユニットテストに加え、クラッシュインジェクションテストを行う。内部実装にのみ公開するチェックポイント機構（例: `ICrashInjector.CheckPoint(name)`）を仕込み、「`Committing`マーカー書き込み直後」「実ファイル操作直後・`Applied`書き込み前」等の要所で強制終了させ、再起動後の`RecoverAsync()`が期待するファイルシステム状態に復旧できることを検証する
