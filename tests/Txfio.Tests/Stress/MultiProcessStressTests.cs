...
public sealed class MultiProcessStressTests
{
    ...
    /// <summary>
    /// 複数プロセスが同じファイルを奪い合っても、最後に Commit したトランザクションの結果だけが残る
    /// </summary>
    /// <remarks>
    /// <para>前提: f0 と f1 だけがある。プロセス数、トランザクション数、シードは環境変数で変えられる</para>
    /// <para>手順: 子プロセスを同時に走らせ、それぞれが 1〜3 パスへの Add / Update / Delete を Commit か Dispose で繰り返す</para>
    ...
    public async Task 複数プロセスの同時更新_最後にコミットした結果だけが残ること()
    {
        ...
        try
        {
            for (int child = 0; child < processes; child++)
            {
                ...
                children.Add(StressProcess.Start(work, child, transactions, (seed * 1000) + child, logFile, startFile));
            }
        ...
        string summary = $"{StressSettings.SeedVariable}={seed} "
    ...
    private sealed record StressRecord(string Token, string Outcome, long Timestamp, IReadOnlyList<(string Path, string Kind)> Operations)
    {
        public static StressRecord Parse(string line)
        {
            string[] fields = line.Split('\t');
            List<(string Path, string Kind)> operations = fields[3]
                .Split(';', StringSplitOptions.RemoveEmptyEntries)
                .Select(operation => operation.Split('='))
                .Select(parts => (parts[0], parts[1]))
                .ToList();
            return new StressRecord(fields[0], fields[1], long.Parse(fields[2], CultureInfo.InvariantCulture), operations);
        }
