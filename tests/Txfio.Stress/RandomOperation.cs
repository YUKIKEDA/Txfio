...
/// <param name="Content">Add と Update で書く内容。それ以外は null</param>
internal sealed record RandomOperation(RandomOperationKind Kind, string Path, string? NewPath, string? Content)
{
    /// <summary>
    ...
    public bool CanApply(IReadOnlyDictionary<string, string> model)
    {
        return Kind switch
        {
    ...
    /// </summary>
    /// <param name="model">相対パスから内容への辞書</param>
    public void ApplyTo(Dictionary<string, string> model)
    {
        switch (Kind)
        {
    ...
    public override string ToString()
    {
        return Kind switch
        {
            RandomOperationKind.Add or RandomOperationKind.Update => $"{Kind}({Path}, \"{Content}\")",
