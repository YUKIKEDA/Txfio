...
変更系（Add / Update / Delete / DeleteTree / Move / Attach / Import）は、パスロックの前にワークフォルダの哨兵を共有で取り、トランザクションが終わるまで持ちます。ディレクトリ Move と `DeleteTreeAsync` のあいだは、その哨兵は排他です。`ReadAsync` と `ExportAsync` はロックしません。このロックは Txfio を使う者同士の協調であり、素の `File` API は止めません。

別のトランザクションが同じパス、またはディレクトリ Move か全削除のあいだのワークフォルダを押さえていると、待たずに `LockContentionException` になります。
...
| 同時実行                  | Txfio 利用者間はパス単位。ディレクトリ Move と全削除のあいだはワークフォルダの哨兵が排他        | スレッドセーフと README にある。ファイルの分離は Read Uncommitted                                    | 書き込みは原則 1 接続。読み取りは WAL などで並行できる                                 |
